...
1. Доставать импорты из hint полей и аннотаций трансформер-функций.
    1. Если есть hint поле, то подгружаем AST текущего файла.
    2. Из рантайма локализируем, где hint поле находится в коде.
    3. Достаем кусок AST, отвечающий за аннотацию.
    4. Достаем строковую репрезентацию аннотации.
    5. Из куска AST забираем все non-literals (названия функций, переменных, модулей, etc.).
    6. Анализируем глобальное именное пространство файла. Локализируем, откуда взялись non-literal переменные, группируем их на:
        1. Локальные (local), которые созданы в текущем файле;
        2. Импортированные (foreign), которые добавлены из другого модуля.
    7. Генерируем строковые репрезентации импортов в зависимости от их принадлежности к группе.
    8. (Потенциально) Разрешаем конфликты импортов. По сути своей их не должно быть при таком подходе и адекватном написании кода.
2. Обязательно перенести default литералы без отношения к оригинальному объекту запроса.
3. ПОЛНОСТЬЮ генерировать код доступа к данным. ПОЛНОСТЬЮ. Чтобы весь код был внутри сгенерированной функции, ВКЛЮЧАЯ запрос и все трансформации. Т.е. мы уходим от фреймворка к codegen библиотеке. Таким образом будет В РАЗЫ проще подключать новые комбинации, и меньше вопросов типа: "а че там под капотом?". Потому что весь код будет НА ЛАДОНИ.
4. Добавить трансформеры в параметры запроса. ЮЗКЕЙС: Например, мы вставляем Pydantic модель в JSONB поле.
5. Убрать возможность переписывания hint'а на запросах, возвращающих Vectorы.

Че делать с `from x import *`?????
Динамические импорты бля. Серьезно нахуй??
нененене
Короче, в рантайме есть importlib. Мы оттуда имеем все ссылки на импортированные модули.
Таким образом, оттуда можно достать последний импортированный модуль, в котором есть искомый символ.
Не без AST магии, но это можно сделать.

```python

@qrk.query
def query(
    arg1: Annotated[QueryParam, Param(hint=list[list[int]])],
    arg2,
    arg3
) -> Annotated[str, Return(shape='one', optional=False)]:
    return f"""
	    SELECT
	    	d AS {column.d(lambda d: ProfileInfoModel(**d) if d is not None else None, hint=ProfileInfoModel | None)}
	    FROM
	    	test_table
	    WHERE
	    	a = {+arg1}
...
def profile_info(d: dict | None) -> ProfileInfoModel | None:
    return ProfileInfoModel(**d) if d is not None else None


@qrk.query
def query2(arg1) -> Annotated[str, Return(same_as=query)]:
    return f"""
	    SELECT
	    	d AS {column.d(profile_info)}
	    FROM
	    	test_table
	    WHERE
	    	a = {+arg1}
...
# Automatically resolve imports and import conflicts
Return(
	shape="one",
    hint=Optional[typing.Literal["one", "two", "three"]]
)


import typing
from typing import Optional
# this goes away due to import conflict resolution:
# from sql.module_ctor import Optional
# from sql.module_ctor import typing



command = "python3 querky generate --backend=asyncpg --datatype=pydantic_v2 sql"
