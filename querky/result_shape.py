    ...
    def set_attributes(self, attrs: tuple[ResultAttribute, ...]):
        ...
        if isinstance(self.annotation, TypeMetaData):
            ...
            self.attribute = ResultAttribute(
                name=attr.name, index=attr.index, type_knowledge=type_knowledge
            )
        else:
            self.attribute = attr
...
class One(ResultShape):
    def __init__(
        self, query: Query, typename: str | None, *, optional: bool = True
    ):
    ...
    def annotate(self):
        self.query.annotation_generator.annotate(
            self.return_type, context="result_type"
        )

    def set_attributes(self, attrs: typing.Tuple[ResultAttribute, ...]):
        for attribute in self.query.query_signature.attributes:
            try:
                if attr_hint := self.query.attr_hints.get(
                    attribute.name, None
                ):
                    attribute.consume_attr(attr_hint)
                self.query.annotation_generator.annotate(
                    attribute.type_knowledge, "attribute"
                )
            except Exception as ex:
                raise QueryInitializationError(
                    self.query, f"attribute `{attribute.name}`"
                ) from ex
        if self.ctor is not None:
...
class Many(One):
    ...
    async def fetch(self, conn, params):
        ...
        if self.ctor.row_factory:
            rows = [self.ctor.row_factory(row) for row in rows]
        return rows
